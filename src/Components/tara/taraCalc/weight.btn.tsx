...
const WeightBtn = ({ btnName, borderColor = 'none', textColor='#000', colorBtn = '#e4e4e4', nameClass, btnKey }: Prop) => {
    const { btn, btn0 } = useStyles({ borderColor, colorBtn, textColor});
    const namedClass = nameClass === 'btn0' ? `${btn} ${btn0}` : `${btn}`
    return (
        <div className={namedClass} data-key={btnKey ? btnKey : btnName}>{btnName}</div>
    )
}
